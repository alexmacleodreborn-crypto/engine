...
soften_Z = st.sidebar.checkbox("Trap Softening Enabled", value=True)
...
with col1:
    ...
    st.line_chart(df[["time", "portal_score"]].set_index("time"))

    st.subheader("Effective Time Rate (dτ_SL/dt)")
    st.line_chart(df[["time", "tau_rate"]].set_index("time"))

with col2:
    st.subheader("Trap Strength Z")
    st.line_chart(df[["time", "Z"]].set_index("time"))

    st.subheader("Time Modulation γ")
    st.line_chart(df[["time", "gamma"]].set_index("time"))
